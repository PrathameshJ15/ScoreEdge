...
function SignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const branchMap: Record<string, { code: string; dept: string }> = {
      comp: { code: 'COMP', dept: 'Computer Engineering' },
      it: { code: 'IT', dept: 'Information Technology' },
      aids: { code: 'AIDS', dept: 'AI & Data Science' },
      entc: { code: 'ENTC', dept: 'Electronics & Telecom' },
    };
    const bInfo = branchMap[branch] || { code: 'COMP', dept: 'Computer Engineering' };
    const result = await signup(
      email,
      password,
      fullName,
      {
        branch_code: bInfo.code,
        department: bInfo.dept,
        academic_year: 'SE',
        year_number: 2,
        semester_number: 4,
